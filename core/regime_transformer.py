"""
Regime-conditioned aerodynamic polar → 50D Fourier-style embedding.

Implements: input embedding, 3 parallel experts, router, MoE fusion,
...
class RegimeConditionedAeroTransformer(nn.Module):
    """
    Polar ``(B, N, 5)`` (Cl, Cd, α, Re, Ma) with padding mask
    (True = padding) → predicted Fourier vector ``(B, 50)``.
